...
export function FilterProvider({ children }) {
    ...
    fetchDashboard("/dashboard/global/filters")
      .then((result) => {
        const agencyList = (result.agencies ?? []).map((name) => ({
          id: name, label: name, commercials: [],
        }));
  ...
  useEffect(() => {
    ...
    fetchDashboard("/dashboard/global/revenue/by-agency", { year: selectedYear })
      .then((result) => {
        ...
        setAgencies((prev) =>
          prev.map((a) => a.id === selectedAgencyId ? { ...a, commercials: comms } : a)
        );
      })
      .catch(() => {});
  ...
  const clearFilters  = () => { setSelectedAgencyId(null); setSelectedCommercialId(null); setSelectedYear(THIS_YEAR); };

  return (
    <FilterContext.Provider value={{
      agencies, selectedAgency, selectedCommercial,
      selectedYear, setSelectedYear,
      setAgency, setCommercial, clearFilters,
  ...
  return (
    <div style={{ position:"relative", minWidth:120 }}>
      <button onClick={() => setOpen((o) => !o)}
        style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8, width:"100%", padding:"5px 9px", background:S.barBg, border:open?S.btnBorderAct:S.btnBorder, borderRadius:S.radius, boxShadow:open?"0 0 0 1px #0078D4":"none", cursor:"pointer", fontFamily:S.font, outline:"none", transition:"border-color .1s" }}
          ...
          <div style={{ position:"absolute", top:"calc(100% + 3px)", left:0, minWidth:"100%", background:S.barBg, border:"1px solid #E1DFDD", borderRadius:S.radius, zIndex:99, boxShadow:S.listShadow, overflow:"hidden" }}>
            {YEAR_OPTIONS.slice().reverse().map((yr) => (
              <div key={yr} onClick={() => { onChange(yr); setOpen(false); }}
                style={{ padding:"8px 12px", fontSize:12, cursor:"pointer", fontFamily:S.font, background:yr===value?S.itemSel:"", color:yr===value?S.itemSelClr:S.valueClr, fontWeight:yr===value?600:400, borderLeft:yr===value?`2px solid ${S.itemSelBdr}`:"2px solid transparent" }}
                onMouseEnter={(e) => { if(yr!==value) e.currentTarget.style.background=S.itemHov; }}
                onMouseLeave={(e) => { if(yr!==value) e.currentTarget.style.background=""; }}
              >
  ...
  return (
    <div style={{ position:"relative", minWidth:172 }}>
      <button onClick={() => { if(!disabled) setOpen((o)=>!o); }}
        style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8, width:"100%", padding:"5px 9px", background:disabled?S.btnBgDis:S.barBg, border:open?S.btnBorderAct:S.btnBorder, borderRadius:S.radius, boxShadow:open?"0 0 0 1px #0078D4":"none", cursor:disabled?"not-allowed":"pointer", fontFamily:S.font, outline:"none", transition:"border-color .1s" }}
...
export function FilterBar({ style }) {
  const { agencies, selectedAgency, selectedCommercial, selectedYear, setSelectedYear, setAgency, setCommercial, clearFilters, isFiltered } = useFilter();
  return (
