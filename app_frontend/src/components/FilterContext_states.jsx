/**
 * FilterContext.jsx — Unified State + Agency → Commercial filter
 *
 * Exports:
 ...
 *   }
 *
 * Data contract — replace AGENCY_DATA / STATES_DATA with your DWH hook:
 *   type Agency = {
 *     id:          string,
 *     label:       string,
 ...
 *   const { selectedAgency, selectedCommercial, selectedState } = useFilter();
 *   const rows = rawData.filter(r => {
 *     if (selectedCommercial && r.commercialId !== selectedCommercial.id) return false;
 *     else if (selectedAgency && r.agencyId !== selectedAgency.id)        return false;
...
// 24 Tunisian governorates (states)
const STATES_DATA = [
...
export function FilterProvider({
  ...
  const setAgency = (id) => {
    ...
    setSelectedCommercialId(null); // reset commercial when agency changes
  };
  ...
  return (
    <FilterContext.Provider value={{
      ...
      setDateFrom,
      setDateTo,
      isFiltered: !!(selectedAgencyId || selectedStateId || dateFrom || dateTo),
    }}>
      {children}
...
const S = {
  ...
  dateClr:      "#7A4F00",
  dateBdr:      "#F5D57A",
  // pills — state (teal/green to distinguish)
  stateBg:      "#DFF6DD",
  stateClr:     "#0E5E0A",
  stateBdr:     "#A7D9A4",
  // clear button
...
export function FilterBar({ style }) {
  const {
    ...
    setAgency, setCommercial, setState,
  ...
  return (
    <div style={{
      ...
      {/* Connector arrow — dims when no agency selected */}
