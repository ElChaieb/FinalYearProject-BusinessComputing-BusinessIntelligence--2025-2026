...
export function FilterProvider({ children }) {
  ...
  const selectedCommercial =
    commercials.find((c) => c.id === selectedCommercialId) ?? null;
  const selectedState =
...
// Hook to access agency filter state (year, state, commercial)
...
// Small year picker used by the filter bars
...
function Dropdown({
  ...
  // Dropdown used for State and Commercial selection in filter bar
  const [open, setOpen] = useState(false);
...
export function FilterBar({ style }) {
  // Render the filter bar UI (Year, State, Commercial selectors)
  const {
    commercials,
    states,
    selectedCommercial,
