/**
 * AgencyFilterContext_states.jsx — Agency Trends scope
 *
 * Filter: Year · State · Commercial
...
export function FilterProvider({ children }) {
  ...
  useEffect(() => {
    fetchDashboard("/dashboard/agency/filters")
      .then((result) => {
        setCommercials((result.commercials ?? []).map((c) => ({ id: String(c.id), label: c.name })));
      })
      .catch(() => {});
  }, []);
  ...
  const clearFilters  = () => { setSelectedCommercialId(null); setSelectedStateId(null); setSelectedYear(THIS_YEAR); };

  return (
    <FilterContext.Provider value={{
      commercials, states: STATES_DATA,
      selectedCommercial, selectedState,
      selectedYear, setSelectedYear,
      setCommercial, setState, clearFilters,
      isFiltered: !!(selectedCommercialId || selectedStateId || selectedYear !== THIS_YEAR),
    }}>
      {children}
...
export function FilterBar({ style }) {
  const { commercials, states, selectedCommercial, selectedState, selectedYear, setSelectedYear, setCommercial, setState, clearFilters, isFiltered } = useFilter();
  return (
