/**
 * FilterContext_states.v1-no-agency.jsx — State + Commercial filter (no Agency)
 *
 * Variant 1 — Agency filter removed.
 * Commercials are a flat list; selecting one does NOT require selecting an agency first.
 * State filter is unchanged.
 *
 * useFilter() returns:
 *   {
 *     commercials:        { id: string, label: string }[],
 *     states:             State[],
 *     selectedCommercial: { id: string, label: string } | null,
 ...
 *   const { selectedCommercial, selectedState } = useFilter();
 *   const rows = rawData.filter(r => {
 *     if (selectedCommercial && r.commercialId !== selectedCommercial.id) return false;
...
// Flat list of all commercials (no agency grouping needed)
...
export function FilterProvider({
  ...
  // No agency guard — any commercial can be selected freely
  ...
  return (
    <FilterContext.Provider value={{
      ...
      setDateTo,
      clearFilters,
      isFiltered: !!(selectedCommercialId || selectedStateId || dateFrom || dateTo),
    }}>
      {children}
...
export function FilterBar({ style }) {
  ...
  return (
    <div style={{
      ...
      {/* ── Commercial — always enabled, no agency dependency ── */}
      <Dropdown
        sublabel="Commercial"
        placeholder="All commercials"
        options={commercials.map(c => ({ id: c.id, label: c.label }))}
