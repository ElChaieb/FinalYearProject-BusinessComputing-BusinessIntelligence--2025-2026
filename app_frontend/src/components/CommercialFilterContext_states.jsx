/**
 * CommercialFilterContext_states.jsx — Commercial Trends scope
 *
 * Filter: Year · State
...
export function FilterProvider({ children }) {
  ...
  const clearFilters  = () => { setSelectedStateId(null); setSelectedYear(THIS_YEAR); };

  return (
    <FilterContext.Provider value={{
      states: STATES_DATA, selectedState,
      selectedYear, setSelectedYear,
      setState, clearFilters,
      isFiltered: !!(selectedStateId || selectedYear !== THIS_YEAR),
    }}>
      {children}
  ...
  return (
    <div style={{ position:"relative", minWidth:172 }}>
      <button onClick={()=>setOpen((o)=>!o)} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8, width:"100%", padding:"5px 9px", background:S.barBg, border:open?S.btnBorderAct:S.btnBorder, borderRadius:S.radius, boxShadow:open?"0 0 0 1px #0078D4":"none", cursor:"pointer", fontFamily:S.font, outline:"none" }}
        onMouseEnter={(e)=>{if(!open)e.currentTarget.style.border=S.btnBorderHov;}} onMouseLeave={(e)=>{if(!open)e.currentTarget.style.border=S.btnBorder;}}>
        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-start" }}>
          <span style={{ fontSize:10, fontWeight:600, letterSpacing:"0.06em", textTransform:"uppercase", lineHeight:1.3, color:S.labelClr }}>{sublabel}</span>
          <span style={{ fontSize:12, lineHeight:1.4, color:selected?S.valueClr:S.mutedClr }}>{selected?selected.label:placeholder}</span>
        </div>
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ flexShrink:0, transform:open?"rotate(180deg)":"none" }}><path d="M1 3l4 4 4-4" stroke="#605E5C" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
      </button>
      {open && (<><div style={{ position:"fixed", inset:0, zIndex:98 }} onClick={()=>setOpen(false)}/>
...
export function FilterBar({ style }) {
  const { states, selectedState, selectedYear, setSelectedYear, setState, clearFilters, isFiltered } = useFilter();
  return (
