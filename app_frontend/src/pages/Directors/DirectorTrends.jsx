/**
 * DirectorTrends.jsx — Trends & Client Base (Director / Global scope)
 *
 * Data sources:
 *   useGlobalTrendsByCategory     → KPI cards + donut charts + breakdown table
 *   useGlobalTrendsClientsByState → state ranking cards (current + prev month)
 */
...
const KpiCard = ({ label, value, sub, trend, accentColor }) => (
  <div style={card}>
    ...
    <p style={{ margin:0, fontSize:28, fontWeight:700, color:accentColor??PBI.textPrimary, lineHeight:1.1 }}>{value}</p>
    {sub   && <p style={{ margin:"3px 0 0", fontSize:12, color:PBI.textMuted }}>{sub}</p>}
        ...
        <div style={{ position:"relative", width:120, height:120, flexShrink:0 }}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip formatter={(val,name) => [`${fmtN(val)} units`, name]} contentStyle={{ fontFamily:PBI.font, fontSize:12, border:`1px solid ${PBI.border}`, borderRadius:2 }} />
            </PieChart>
          </ResponsiveContainer>
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", pointerEvents:"none" }}>
            <span style={{ fontSize:13, fontWeight:700, color:PBI.textPrimary, lineHeight:1.1 }}>{fmtN(total)}</span>
            <span style={{ fontSize:9, color:PBI.textMuted, marginTop:2 }}>units</span>
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:5, minWidth:0, flex:1 }}>
          {donutData.map((d,i) => (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:5 }}>
              <span style={{ width:8, height:8, borderRadius:2, flexShrink:0, background:d.color, display:"inline-block" }} />
              <span style={{ fontSize:10, color:PBI.textMuted, whiteSpace:"nowrap", flex:1 }}>{d.name}</span>
              <strong style={{ fontSize:10, color:PBI.textPrimary, paddingLeft:4 }}>{total ? ((d.value/total)*100).toFixed(1) : 0}%</strong>
            </div>
          ))}
        </div>
      </div>
...
const ModelsBarCard = ({ categories }) => {
  ...
  const modelBarData = categories.map((cat) => {
    ...
    for (let i = 0; i < MAX_MODELS; i++) {
      entry[`slot_${i}`]     = cat.models[i] ? cat.models[i].months.reduce((s,m) => s+(m.n??0), 0) : 0;
    ...
    <div style={{ ...card, display:"flex", flexDirection:"column" }}>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={modelBarData} barCategoryGap="30%" margin={{ top:4, right:8, left:-12, bottom:0 }}>
          ...
          <XAxis dataKey="name" tick={{ fontSize:10, fill:PBI.textMuted, fontFamily:PBI.font }} axisLine={{ stroke:PBI.border }} tickLine={false} />
          <YAxis tick={{ fontSize:10, fill:PBI.textMuted, fontFamily:PBI.font }} axisLine={{ stroke:PBI.border }} tickLine={false} />
          <Tooltip cursor={{ fill:"rgba(17,141,255,.06)" }} contentStyle={{ fontFamily:PBI.font, fontSize:12, border:`1px solid ${PBI.border}`, borderRadius:2 }} />
          {Array.from({ length:MAX_MODELS }, (_,slotIdx) => (
            <Bar key={slotIdx} dataKey={`slot_${slotIdx}`} stackId="a" maxBarSize={40} radius={slotIdx===MAX_MODELS-1?[2,2,0,0]:[0,0,0,0]}>
              {modelBarData.map((entry,di) => <Cell key={di} fill={entry._color} fillOpacity={SLOT_OPACITIES[slotIdx]??0.3} />)}
            </Bar>
          ))}
        </BarChart>
      </ResponsiveContainer>
      <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginTop:8 }}>
        {categories.map((cat) => (
          <div key={cat.id} style={{ display:"flex", alignItems:"center", gap:5 }}>
            ...
            <span style={{ fontSize:10, color:PBI.textMuted }}>{cat.label}</span>
          </div>
        ))}
      </div>
    </div>
...
const StateRankCard = ({ title, subtitle, accentColor, states }) => (
  <div style={card}>
    ...
    <p style={{ margin:"0 0 14px", fontSize:11, color:PBI.textMuted }}>{subtitle}</p>
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      {states.map((s,i) => {
        const barPct = states[0]?.orders > 0 ? (s.orders/states[0].orders)*100 : 0;
        return (
          <div key={s.id}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:4 }}>
              <span style={{ fontSize:12, color:PBI.textPrimary, display:"flex", alignItems:"center", gap:6 }}>
                <span style={{ fontSize:10, fontWeight:700, color:"#fff", background:accentColor, borderRadius:2, width:16, height:16, display:"inline-flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{i+1}</span>
                {s.label}
              </span>
              <strong style={{ fontSize:13, color:accentColor }}>{fmtN(s.orders)}</strong>
            </div>
            <div style={{ height:5, borderRadius:3, background:"#F3F2F1", overflow:"hidden" }}>
              <div style={{ height:"100%", borderRadius:3, width:`${barPct}%`, background:accentColor, opacity:0.85 }} />
            </div>
          </div>
        );
      })}
  ...
  return (
    <div style={{ ...card, padding:"20px 24px", overflowX:"auto" }}>
      ...
      <table style={{ borderCollapse:"collapse", fontFamily:PBI.font, fontSize:12, minWidth:"100%" }}>
        <thead>
          <tr>
            <th style={{ ...thBase, textAlign:"left", minWidth:170, position:"sticky", left:0, zIndex:3 }}>Category / Model</th>
        ...
        <tbody>
            ...
            return (
              <>
                <tr key={cat.id} onClick={() => toggleCat(cat.id)} style={{ cursor:"pointer", background:"#F7F6F5" }} onMouseEnter={(e) => (e.currentTarget.style.background="#EDEBE9")} onMouseLeave={(e) => (e.currentTarget.style.background="#F7F6F5")}>
                  <td style={{ padding:"6px 10px", fontSize:12, fontWeight:700, color:PBI.textPrimary, whiteSpace:"nowrap", borderBottom:`1px solid ${PBI.border}`, borderRight:`1px solid ${PBI.border}`, background:"inherit", position:"sticky", left:0, zIndex:2 }}>
                    ...
                    {cat.label}
                    {catPct && <span style={{ marginLeft:8, fontSize:10, fontWeight:500, color:catPct.up?PBI.green:PBI.red }}>{catPct.up?"▲":"▼"} {catPct.label}</span>}
                  </td>
                  {catMonths.map((cm,mi) => (
                    <>
                      <td key={`cat-${cat.id}-${mi}-nm1`} style={tdNum(false,true,{borderLeft:`1px solid ${PBI.border}`,fontWeight:600,background:"#F7F6F5"})}>{fmtN(cm.nMinus1)}</td>
                      <td key={`cat-${cat.id}-${mi}-n`}   style={tdNum(false,false,{fontWeight:600,background:"#F7F6F5"})}>{fmtN(cm.n)}</td>
                    </>
                  ))}
                  <td style={tdNum(true,true,{borderLeft:`2px solid ${PBI.border}`,fontWeight:700,background:"#EDEBE9"})}>{fmtN(catTotalNM1)}</td>
                  <td style={tdNum(true,false,{fontWeight:700,background:"#EDEBE9"})}>{fmtN(catTotalN)}</td>
                </tr>
                  ...
                  return (
                    <tr key={`${cat.id}-${mod.model}`} style={{ background:rowBg }} onMouseEnter={(e) => (e.currentTarget.style.background="#F3F2F1")} onMouseLeave={(e) => (e.currentTarget.style.background=rowBg)}>
                      ...
                      {mod.months.map((mm,mi) => (
                        <>
                          <td key={`mod-${mod.model}-${mi}-nm1`} style={tdNum(false,true,{borderLeft:`1px solid ${PBI.border}`})}>{fmtN(mm.nMinus1)}</td>
                          <td key={`mod-${mod.model}-${mi}-n`}   style={tdNum(false,false)}>{fmtN(mm.n)}</td>
                        </>
                      ))}
                      <td style={tdNum(true,true,{borderLeft:`2px solid ${PBI.border}`})}>{fmtN(modTotalNM1)}</td>
          ...
          <tr style={{ background:"#DEECF9" }}>
            ...
            {monthlyTotals.map((mt,mi) => (
              <>
                <td key={`gt-${mi}-nm1`} style={tdNum(false,true,{borderLeft:`1px solid ${PBI.border}`,fontWeight:700,background:"#DEECF9"})}>{fmtN(mt.nMinus1)}</td>
                <td key={`gt-${mi}-n`}   style={tdNum(false,false,{fontWeight:700,background:"#DEECF9"})}>{fmtN(mt.n)}</td>
              </>
            ))}
            <td style={{ padding:"6px 10px", fontSize:12, fontWeight:700, textAlign:"right", whiteSpace:"nowrap", borderBottom:`1px solid ${PBI.border}`, borderRight:`1px solid ${PBI.border}`, borderLeft:`2px solid ${PBI.border}`, color:COLOR_NM1, background:"#C7E0F4" }}>{fmtN(totalNM1)}</td>
            <td style={{ padding:"6px 10px", fontSize:12, fontWeight:700, textAlign:"right", whiteSpace:"nowrap", borderBottom:`1px solid ${PBI.border}`, borderRight:`1px solid ${PBI.border}`, color:COLOR_N, background:"#C7E0F4" }}>{fmtN(totalN)}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
function TrendsPage() {
  ...
  const catTotals  = useMemo(() => categories.map((cat) => ({
    ...
    n:   cat.models.flatMap((m) => m.months).reduce((s,mo) => s+(mo.n??0),0),
    nM1: cat.models.flatMap((m) => m.months).reduce((s,mo) => s+(mo.nMinus1??0),0),
  })), [categories]);
  ...
  const statesCur  = useMemo(() => buildStateOrders(stateCurData?.rows  ?? [], "n"), [stateCurData]);
  const statesPrev = useMemo(() => buildStateOrders(statePrevData?.rows ?? [], "n"), [statePrevData]);
  ...
  const bot5Cur    = [...statesCur].sort((a,b) => a.orders-b.orders).slice(0,5).reverse();
  const bot5Prev   = [...statesPrev].sort((a,b) => a.orders-b.orders).slice(0,5).reverse();
  ...
  return (
    <div style={{ background:PBI.pageBg, minHeight:"100vh", padding:"20px 24px", fontFamily:PBI.font }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:12, marginBottom:16, alignItems:"stretch" }}>
        ...
        {highest && (
          <div style={card}>
            <p style={{ margin:"0 0 6px", fontSize:11, color:PBI.textMuted, textTransform:"uppercase", letterSpacing:"0.05em" }}>Highest Selling Category</p>
            <div style={{ display:"flex", alignItems:"center", gap:7, margin:"8px 0 5px" }}>
              <span style={{ display:"inline-block", width:11, height:11, borderRadius:2, background:highest.color, flexShrink:0 }} />
        ...
        {lowest && (
          <div style={card}>
            <p style={{ margin:"0 0 6px", fontSize:11, color:PBI.textMuted, textTransform:"uppercase", letterSpacing:"0.05em" }}>Lowest Selling Category</p>
            <div style={{ display:"flex", alignItems:"center", gap:7, margin:"8px 0 5px" }}>
              <span style={{ display:"inline-block", width:11, height:11, borderRadius:2, background:lowest.color, flexShrink:0 }} />
      ...
      <div style={{ marginTop:16 }}>
        <p style={{ margin:"0 0 12px", fontSize:13, fontWeight:600, color:PBI.textPrimary }}>State Rankings</p>
        {(l2||l3) ? <Spinner /> : (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:12 }}>
            <StateRankCard title="Top 5 States"    subtitle={`Highest orders · ${MONTHS_FULL[CUR_MONTH_IDX]} (current month)`}  accentColor="#107C10" states={top5Cur}  />
            <StateRankCard title="Top 5 States"    subtitle={`Highest orders · ${MONTHS_FULL[PREV_MONTH_IDX]} (last month)`}    accentColor="#118DFF" states={top5Prev} />
            <StateRankCard title="Bottom 5 States" subtitle={`Lowest orders · ${MONTHS_FULL[CUR_MONTH_IDX]} (current month)`}   accentColor="#D13438" states={bot5Cur}  />
            <StateRankCard title="Bottom 5 States" subtitle={`Lowest orders · ${MONTHS_FULL[PREV_MONTH_IDX]} (last month)`}     accentColor="#E66C37" states={bot5Prev} />
          </div>
        )}
      </div>
    </div>
