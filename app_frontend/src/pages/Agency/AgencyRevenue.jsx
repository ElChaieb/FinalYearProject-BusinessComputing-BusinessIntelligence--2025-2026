/**
 * AgencyRevenue.jsx — Revenue Dashboard (Agency Manager scope)
 *
 * Data sources:
 *   useAgencyRevenueMonthly      → KPI cards + bar chart
 *   useAgencyRevenueByCategory   → breakdown table
 *   useAgencyRevenueByCommercial → 4 commercial donut cards
 */
...
import {
  ...
  useAgencyRevenueByCategory,
  useAgencyRevenueByCommercial,
  buildCategories,
  buildEntityRevenue,
...
const PBITooltip = ({active,payload,label}) => {
  ...
  return <div style={{background:"#fff",border:`1px solid ${PBI.border}`,borderRadius:2,padding:"10px 14px",fontFamily:PBI.font,boxShadow:"0 4px 16px rgba(0,0,0,.18)",fontSize:12,pointerEvents:"none"}}>{label&&<p style={{margin:"0 0 6px",fontWeight:600,color:PBI.textPrimary}}>{label}</p>}{payload.map((e,i)=><p key={i} style={{margin:"2px 0",color:e.fill}}><span style={{color:PBI.textMuted}}>{e.name}: </span><strong>{fmt(e.value)}</strong></p>)}</div>;
};

const DonutCard = ({title,subtitle,slices}) => {
      ...
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        <div style={{position:"relative",width:120,height:120,flexShrink:0}}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart><Pie data={slices} cx="50%" cy="50%" innerRadius="54%" outerRadius="78%" dataKey="value" paddingAngle={2} startAngle={90} endAngle={-270}>{slices.map((d,i)=><Cell key={i} fill={d.color} stroke="none"/>)}</Pie><Tooltip formatter={(val,name)=>[fmtShort(val),name]} contentStyle={{fontFamily:PBI.font,fontSize:11,border:`1px solid ${PBI.border}`,borderRadius:2}}/></PieChart>
          </ResponsiveContainer>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6,minWidth:0,flex:1}}>
          {slices.map((d,i) => { const share=total?((d.value/total)*100).toFixed(1):0; return (
            <div key={i} style={{display:"flex",alignItems:"center",gap:6}}>
              ...
              <div style={{minWidth:0,flex:1}}>
                ...
                <div style={{height:3,background:"#F3F2F1",borderRadius:2,marginTop:2}}><div style={{height:"100%",borderRadius:2,width:`${share}%`,background:d.color,transition:"width .3s ease"}}/></div>
              </div>
            </div>
          );})}
        </div>
        ...
        <table style={{borderCollapse:"collapse",fontFamily:PBI.font,fontSize:12,minWidth:"100%"}}>
          ...
          <tbody>
            {categories.map((cat)=>{
              ...
              const catTotalN=catMonths.reduce((s,m)=>s+m.n,0),catTotalNM1=catMonths.reduce((s,m)=>s+m.nMinus1,0);
              return (<>
                <tr key={cat.id} onClick={()=>toggleCat(cat.id)} style={{cursor:"pointer",background:"#F7F6F5"}} onMouseEnter={(e)=>(e.currentTarget.style.background="#EDEBE9")} onMouseLeave={(e)=>(e.currentTarget.style.background="#F7F6F5")}>
                  <td style={{padding:"6px 10px",fontSize:12,fontWeight:700,color:PBI.textPrimary,whiteSpace:"nowrap",verticalAlign:"middle",borderBottom:`1px solid ${PBI.border}`,borderRight:`1px solid ${PBI.border}`,background:"inherit",position:"sticky",left:0,zIndex:2}}>
                    <span style={{display:"inline-block",width:9,height:9,borderRadius:2,background:cat.color,marginRight:7,verticalAlign:"middle"}}/><span style={{marginRight:6,fontSize:10,color:PBI.textMuted}}>{isOpen?"▼":"▶"}</span>{cat.label}
                  </td>
                  {catMonths.map((cm,mi)=>(<><td key={`cat-${cat.id}-${mi}-nm1`} style={tdNum(false,true,{borderLeft:`1px solid ${PBI.border}`,fontWeight:600,background:"#F7F6F5"})}>{fmt(cm.nMinus1)}</td><td key={`cat-${cat.id}-${mi}-n`} style={tdNum(false,false,{fontWeight:600,background:"#F7F6F5"})}>{fmt(cm.n)}</td></>))}
                ...
                {isOpen&&cat.models.map((mod,ri)=>{const modTotalN=mod.months.reduce((s,m)=>s+(m.n??0),0),modTotalNM1=mod.months.reduce((s,m)=>s+(m.nMinus1??0),0),rowBg=ri%2===0?"#fff":"#FAFAF9";return(
                  <tr key={`${cat.id}-${mod.model}`} style={{background:rowBg}} onMouseEnter={(e)=>(e.currentTarget.style.background="#F3F2F1")} onMouseLeave={(e)=>(e.currentTarget.style.background=rowBg)}>
                    ...
                    {mod.months.map((mm,mi)=>(<><td key={`mod-${mod.model}-${mi}-nm1`} style={tdNum(false,true,{borderLeft:`1px solid ${PBI.border}`})}>{fmt(mm.nMinus1)}</td><td key={`mod-${mod.model}-${mi}-n`} style={tdNum(false,false)}>{fmt(mm.n)}</td></>))}
                    <td style={tdNum(true,true,{borderLeft:`2px solid ${PBI.border}`})}>{fmt(modTotalNM1)}</td><td style={tdNum(true,false)}>{fmt(modTotalN)}</td>
                  </tr>
                );})}
              </>);
            })}
            <tr style={{background:"#DEECF9"}}>
              ...
              {monthlyTotals.map((mt,mi)=>(<><td key={`gt-${mi}-nm1`} style={tdNum(false,true,{borderLeft:`1px solid ${PBI.border}`,fontWeight:700,background:"#DEECF9"})}>{fmt(mt.nMinus1)}</td><td key={`gt-${mi}-n`} style={tdNum(false,false,{fontWeight:700,background:"#DEECF9"})}>{fmt(mt.n)}</td></>))}
...
function RevenueInner() {
  ...
  const monthlyTotals = useMemo(()=>{
    ...
    return MONTHS.map((abbr,i)=>{ const row=rows.find((r)=>r.month===i+1)??{}; return {abbr,full:MONTHS_FULL[i],n:Number(row.n??0),nMinus1:Number(row.n_minus1??0)}; });
  },[monthlyData]);

  const categories   = useMemo(()=>buildCategories(categoryData?.rows??[]),[categoryData]);
  const commercials  = useMemo(()=>buildEntityRevenue(commData?.rows??[],"full_name","user_id"),[commData]);

  const now=new Date(), curIdx=now.getMonth(), prevIdx=curIdx>0?curIdx-1:11;
  const totalN    = monthlyTotals.reduce((s,m)=>s+m.n,0);
  const totalNM1  = monthlyTotals.reduce((s,m)=>s+m.nMinus1,0);
    ...
    <div style={{background:PBI.pageBg,minHeight:"100vh",padding:"20px 24px",fontFamily:PBI.font}}>
      ...
      <div style={{marginTop:16}}>
        <p style={{margin:"0 0 10px",fontSize:13,fontWeight:600,color:PBI.textPrimary,fontFamily:PBI.font}}>
          Revenue by Commercial <span style={{fontSize:11,fontWeight:400,color:PBI.textMuted,marginLeft:8}}>Share of total revenue across periods</span>
        </p>
...
export default function ARevenue() {
