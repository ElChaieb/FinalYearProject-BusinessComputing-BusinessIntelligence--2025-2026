...
// ─── Mock commercial (salesman) revenue data ───────────────────────────────────
// Replace with your DWH hook. Each commercial has revenue per month for n and nMinus1.
// Index 0 = Jan, 11 = Dec.
const COMMERCIALS = [
  {
    id: "comm_ahmed",
...
// ─── Commercial donut row — 4 cards ─────────────────────────────────────────────
const CommercialDonutRow = () => {
  ...
  const currentMonthSlices = makeSlices((c) => c.months[curIdx].n);
  // Last month (prevIdx)   — Year n
  const lastMonthSlices = makeSlices((c) => c.months[prevIdx].n);
  // Current year (n)       — sum all months
  const currentYearSlices = makeSlices((c) =>
    c.months.reduce((s, m) => s + m.n, 0),
  );
  // Last year (n-1)        — sum all months
  const lastYearSlices = makeSlices((c) =>
    c.months.reduce((s, m) => s + m.nMinus1, 0),
  );

  const CARDS = [
    {
      title: "Commercial Share — Current Month",
      subtitle: `${MONTHS_FULL[curIdx]} ${THIS_YEAR}`,
      slices: currentMonthSlices,
    },
    {
      title: `Commercial Share — Year ${THIS_YEAR}`,
      subtitle: "Full year to date",
      slices: currentYearSlices,
    },
    {
      title: "Commercial Share — Last Month",
      subtitle: `${MONTHS_FULL[prevIdx]} ${THIS_YEAR}`,
      slices: lastMonthSlices,
    },
    {
      title: `Commercial Share — Year ${LAST_YEAR}`,
