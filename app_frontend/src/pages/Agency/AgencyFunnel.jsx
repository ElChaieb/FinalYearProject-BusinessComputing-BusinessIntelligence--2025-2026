...
import {
  FilterProvider,
  FilterBar,
} from "../../components/AgencyFilterContext";
import { useFilter } from "../../components/AgencyFilterContext";
import Layout from "../../components/Layout";
import {
  useAgencyFunnelMonthly,
  useAgencyFunnelByCommercial,
  buildFunnelData,
  buildFunnelByEntity,
...
const PREVIOUS_MONTH_LABEL =
  MONTHS[now.getMonth() > 0 ? now.getMonth() - 1 : 11];
...
const ConvTable = ({ data }) => {
  ...
  const enriched = data.map((col) => ({
    ...col,
    oqRate:
      col.oppoWon || col.oppoLost
        ? +((col.oppoWon / (col.oppoWon + col.oppoLost)) * 100).toFixed(1)
        : null,
    qsRate:
      col.quoteWon || col.quoteLost
        ? +((col.quoteWon / (col.quoteWon + col.quoteLost)) * 100).toFixed(1)
        : null,
  }));
  return (
          ...
          <tr>
            ...
            {data.map((col) => {
              const isYear = col.period.toLowerCase().includes("year"),
                isPrev = col.period.includes("n-1");
              return (
                <th
                  key={col.period}
                  style={{
              ...
              >
                ...
                <td
                  style={{
                    ...
                    paddingLeft: !isFirst && row.type !== "rate" ? 22 : 10,
                    borderBottom: `1px solid ${PBI.border}`,
                    borderRight: `1px solid ${PBI.border}`,
                    background: "#fff",
                    whiteSpace: "nowrap",
                ...
                {enriched.map((col) => {
                  const val = col[row.key],
                    isYear = col.period.toLowerCase().includes("year");
...
const DonutCard = ({ title, commercials, valueKey }) => {
  const donutData = commercials.map((c) => ({
    name: c.name,
    value: c[valueKey] || 0,
  }));
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: 8, color: PBI.textMuted, marginTop: 1 }}>
              total
            </span>
          </div>
        </div>
...
function FunnelPageInner() {
  ...
  const {
    ...
    loading: l1,
    error: e1,
  } = useAgencyFunnelMonthly(selectedYear, commercialId);
  const {
    data: funnelComm,
    loading: l2,
    error: e2,
  } = useAgencyFunnelByCommercial(selectedYear, commercialId);

  const data = useMemo(
    () => buildFunnelData(funnelMonthly?.rows ?? []),
    [funnelMonthly],
  );
  const { yearN: commsN, yearNm1: commsNm1 } = useMemo(
    () => buildFunnelByEntity(funnelComm ?? {}),
    [funnelComm],
  );
  ...
  const funnelData = [
    ...
    {
      name: "Quotes",
      value: (yearN.quoteWon || 0) + (yearN.quoteLost || 0),
      fill: PBI.colors[2],
    },
    { name: "Sales Won", value: yearN.saleWon || 0, fill: PBI.colors[7] },
  ];
  const oqRateN =
    yearN.oppoWon || yearN.oppoLost
      ? ((yearN.oppoWon / (yearN.oppoWon + yearN.oppoLost)) * 100).toFixed(1)
      : "—";
  const qsRateN =
    yearN.quoteWon || yearN.quoteLost
      ? ((yearN.quoteWon / (yearN.quoteWon + yearN.quoteLost)) * 100).toFixed(1)
      : "—";
  const oppoCompare = MONTHS.map((m) => {
    const p = data.find((d) => d.period === `${m} n-1`) || {},
      c = data.find((d) => d.period === m) || {};
    return { month: m, "n-1": p.oppoWon || 0, n: c.oppoWon || 0 };
  });
  const quoteCompare = MONTHS.map((m) => {
    const p = data.find((d) => d.period === `${m} n-1`) || {},
      c = data.find((d) => d.period === m) || {};
    return { month: m, "n-1": p.quoteWon || 0, n: c.quoteWon || 0 };
  });
  ...
  const DONUT_ROWS = [
    {
      ...
      yearLabel: `Year ${YEAR_NM1}`,
      commercials: commsNm1.map((c) => ({
        ...c,
        oppoTotal: c.oppoWon + c.oppoLost,
        quoteTotal: c.quoteWon + c.quoteLost,
      })),
      cards: [
    ...
    {
      rowLabel: "Opportunities",
      yearLabel: `Year ${YEAR_N}`,
      commercials: commsN.map((c) => ({
        ...c,
        oppoTotal: c.oppoWon + c.oppoLost,
        quoteTotal: c.quoteWon + c.quoteLost,
      })),
  ...
  return (
    ...
    >
        ...
        >
          ...
          <KpiCard
            ...
            value={
              yearN.oppoWon && yearN.oppoLost
                ? +(
                    (yearN.oppoWon / (yearN.oppoWon + yearN.oppoLost)) *
                    100
                  ).toFixed(1) + "%"
                : "—"
            }
          ...
          />
          <KpiCard
            label={`Opportunities Won (${YEAR_NM1})`}
            value={fmtN(yearPrev.oppoWon)}
            delta={null}
            positive={true}
          />
          <KpiCard
            label={`Opportunities Lost (${YEAR_NM1})`}
            value={fmtN(yearPrev.oppoLost)}
            delta={null}
            positive={false}
          />
          <KpiCard
            label={`Oppos Conv. Rate (${YEAR_NM1})`}
            value={
              yearPrev.oppoWon && yearPrev.oppoLost
                ? +(
                    (yearPrev.oppoWon /
                      (yearPrev.oppoWon + yearPrev.oppoLost)) *
                    100
          ...
          <KpiCard
            ...
            value={
              yearN.quoteWon && yearN.quoteLost
                ? +(
                    (yearN.quoteWon / (yearN.quoteWon + yearN.quoteLost)) *
                    100
                  ).toFixed(1) + "%"
                : "—"
            }
          ...
          />
          <KpiCard
            label={`Quotes Won (${YEAR_NM1})`}
            value={fmtN(yearPrev.quoteWon)}
            delta={null}
            positive={true}
          />
          <KpiCard
            label={`Quotes Lost (${YEAR_NM1})`}
            value={fmtN(yearPrev.quoteLost)}
            delta={null}
            positive={false}
          />
          <KpiCard
            label={`Quotes Conv. Rate (${YEAR_NM1})`}
            value={
              yearPrev.quoteWon && yearPrev.quoteLost
                ? +(
                    (yearPrev.quoteWon /
                      (yearPrev.quoteWon + yearPrev.quoteLost)) *
                    100
      ...
      >
        {[
          { title: "Opportunities Won · n-1 vs n", data: oppoCompare },
          { title: "Quotes Won · n-1 vs n", data: quoteCompare },
        ].map(({ title, data: cd }) => (
          <div key={title} style={card}>
            ...
            <p
              style={{ margin: "0 0 8px", fontSize: 11, color: PBI.textMuted }}
            >
              By month
            </p>
      ...
      <div style={{ ...card, padding: "20px 24px" }}>
        ...
        <p style={{ margin: "0 0 12px", fontSize: 11, color: PBI.textMuted }}>
          Metrics as rows · Periods as columns · Rates auto-calculated
        </p>
