...
def _sync_users(opdb_cur, dwh_cur) -> dict:
    ...
    values = [
        ...
        (
            ...
            MAIN_AGENCY_ID,            # agency_id — not available in Op DB

            MAIN_AGENCY_NAME,  # agency_name

            MAIN_AGENCY_REGION,  # region
...
def _sync_opportunities(opdb_cur, dwh_cur) -> dict:
    ...
    Mapping:
      ...
      - agency_id : NULL (main agency not mapped here; set via dim_user if needed)
    ...
    opdb_cur.execute("""
        SELECT o.oppo_id, o.channel, u.crm_user_id,
               o.created_date, o.city, o.deleted, o.source,
    ...
    for r in new_rows:
        ...
        values.append((
            ...
            MAIN_AGENCY_ID,                          # agency_id — not in Op DB
...
def _sync_quotes(opdb_cur, dwh_cur) -> dict:
    ...
    opdb_cur.execute("""
        SELECT q.quote_id_crm, q.oppo_id, q.ar_ref,
               q.created_date, u.crm_user_id, q.source
        FROM quotes q
...
def _sync_sales(opdb_cur, dwh_cur) -> dict:
    ...
    opdb_cur.execute("""
        SELECT s.sale_id, s.oppo_id, u.crm_user_id,
...
def _sync_targets(opdb_cur, dwh_cur) -> dict:
    ...
    for r in rows:
        ...
        values.append((
            ...
            MAIN_AGENCY_ID,          # agency_id — Akouda
...
def sync_opdb_to_dwh() -> dict:
    ...
    try:
        ...
        # Ensure the main agency row exists before any sync

        dwh_cur.execute("""

        INSERT INTO dim_agency (agency_id, name, city, region)

        VALUES (%s, %s, %s, %s)

        ON CONFLICT (agency_id) DO NOTHING

        """, (MAIN_AGENCY_ID, MAIN_AGENCY_NAME, MAIN_AGENCY_NAME, MAIN_AGENCY_REGION))
