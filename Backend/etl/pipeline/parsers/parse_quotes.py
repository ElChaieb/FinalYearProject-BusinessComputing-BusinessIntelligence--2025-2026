...
Business rules:
    - Quotes without a numeric `Quot_OrderQuoteID` are skipped.
    - Quotes with null or missing `Quot_opportunityid` are skipped.
    - A quote is considered "won" if its id appears in the SALES sheet;
        `deleted` stored as the inverse (i.e. deleted = not is_won).
    - User resolution prefers `User_UserId`; if missing, name-based lookup is
        attempted via `resolve_user_id_by_name`.
    - Vehicle base price is looked up from `dim_vehicle` if available.
"""
import pandas as pd
...
def parse_and_load_quotes(df_devis: pd.DataFrame, df_sales: pd.DataFrame | None = None) -> dict:
    # Main parser for quotes; also looks up won quote IDs from SALES.
...
def _resolve_user(row, has_user_id: bool, cur, cache: dict, quote_id: int):
    # Resolve user id for a quote either by direct User_UserId or by name.
    if has_user_id:
